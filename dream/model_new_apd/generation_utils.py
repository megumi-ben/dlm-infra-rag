...
class DreamGenerationMixin:
    ...
    def initialize_with_kickstart(
        ...
        # 重新计算 Mask 比例 (对 APD 来说这个比例用处不大，但保留接口一致性)
    ...
    def _sample(
        ...
        # [新增] 获取 pad_token_id，用于 initialize_with_kickstart
        ...
        # 1. 初始化画布 (Pad input_ids to max_length)
        # [逻辑调整] 这里保持原有的 F.pad 逻辑
        x = F.pad(input_ids, (0, max_length - input_ids.shape[1]), value=mask_token_id)

        # 2. [关键调整] 提前处理 Attention Mask
        # 原代码 Attention Mask 处理在后面，我们需要提上来，因为 initialize_with_kickstart 需要用到它
        ...
        # 3. [新增] 调用 Kickstart 初始化
        prompt_len = input_ids.shape[1]
        x, initial_mask_ratio, _ = self.initialize_with_kickstart(
            x=x,
        ...
        # 4. [新增] 自适应步数调度 (Adaptive Scheduling)
        # 根据剩余 Mask 的比例动态计算需要的步数
        effective_steps = int(steps * initial_mask_ratio)
        if effective_steps < 1: effective_steps = 1
        
        # [新增] 时间步重映射
        # 即使只剩 10% 的 Mask，我们也认为是从 t=0.1 开始去噪，而不是从 t=1.0 开始
        ...
        for i in range(effective_steps):
            ...
            # [关键] Dream Logit Shift 修正 (原代码有这行，保留)
            logits = torch.cat([logits[:,:1], logits[:, :-1]], dim=1)
            ...
            if alg == 'origin':
                ...
                # 处理其他算法 (maskgit_plus, topk_margin, entropy)
                if alg == 'maskgit_plus':
                ...
                # 计算当前步骤需要 Transfer 多少 Token
                num_mask_token = mask_index.sum()
                # [修改] 判断条件改为 effective_steps - 1
                number_transfer_tokens = int(num_mask_token * (1 - s / t)) if i < effective_steps - 1 else num_mask_token
    ...
    def apd_sample(
        ...
        max_length = generation_config.max_length
        mask_token_id = generation_config.mask_token_id
        pad_token_id = generation_config.pad_token_id # 确保 Config 里有这个，或者从 self.config 获取
        temperature = generation_config.temperature
        top_p = generation_config.top_p
        top_k = generation_config.top_k
        ...
        # 2. [关键] 提前构造 Attention Mask
        # 原代码 Attention Mask 构造在更后面，我们需要提上来，因为 initialize_with_kickstart 可能需要计算 Confidence
        if attention_mask is not None and torch.any(attention_mask == 0.0):
            attention_mask = F.pad(attention_mask, (0, max_length - attention_mask.shape[1]), value=1.0)
            tok_idx = attention_mask.long().cumsum(-1) - 1
            tok_idx.masked_fill_(attention_mask == 0, 1)
        ...
        # 3. [新增] 调用 Kickstart 初始化
        # 这一步会将 Draft 填入 x，并根据置信度把不可信的 Token 变回 [MASK]
        x, _, kickstart_nfe = self.initialize_with_kickstart(
            x=x,
            prompt_len=prompt_len,
            kickstart_ids=kickstart_ids,
            kickstart_strength=kickstart_strength,
        ...
        num_forward_evals += kickstart_nfe

        # 4. [新增/关键] APD 专用截断逻辑 (Prefix Enforcement)
        # 这里的 initialize_with_kickstart 可能会产生类似 [Token, Token, Mask, Token, Mask] 的结果
        # 但 APD 是顺序生成的，无法处理中间的 Mask。
        # 我们必须找到第一个 [MASK] 的位置，并将它之后的所有 Token 强制 Mask 掉。
        
        # 找到生成区域的第一个 Mask
        gen_x = x[:, prompt_len:]
        mask_pos = (gen_x == mask_token_id).int().argmax(dim=1) # 获取第一个 mask 的索引
        
        # 处理全满或全空的边界情况
        has_mask = (gen_x == mask_token_id).any(dim=1)
        
        for b in range(x.shape[0]):
            if has_mask[b]:
                first_mask_idx = prompt_len + mask_pos[b].item()
                # 强制将第一个 Mask 之后的所有位置重置为 Mask
                x[b, first_mask_idx:] = mask_token_id
                # 设定当前起始位置为第一个 Mask
                # (注意：APD 目前似乎只支持 batch_size=1 的逻辑比较完善，这里假设 batch 维度一致)
                start_idx = first_mask_idx
            else:
                # 如果没有 Mask (Draft 完美)，则指向结束，循环将直接跳过
                start_idx = x.shape[1]
        
        # 设定 curr_idx
        curr_idx = start_idx if has_mask.any() else x.shape[1]
