    ...
    try:
        ...
        if dataset_name == 'mbpp':
            ...
            for item in merged_dataset:
                ...
                canonical_solution = item.get('canonical_solution', '').strip()
                test = item.get('test', '').strip()
                entry_point = item.get('entry_point', '').strip()
                # 拼接 target，分段
                target = (
        ...
        for item in merged_dataset:
            try:
                ...
                if dataset_name == 'mmlu':
                    ...
                    t = f"{options[item['answer']]}. "+item['choices'][item['answer']]
                    ...
                    # 问题+选项
                    p = f"{item.get('Question', '').strip()}\n{format_choices(shuffled_choices)}\n"
                    # 答案解析
                    explanation = item.get('Explanation', '').strip()
                    answer_text = shuffled_choices[answer_idx]
...
def split_tarin_test(dataset_name, data_path, data_ratio=1.0, max_size=None):
    if dataset_name=='e2e':
    ...
    if max_size is not None:
        max_count = min(total, int(max_size))
        print(f"仅使用前 {max_count} 条数据（max_size={max_size}）参与划分。")
...
def process_prompt(prompt_text: str, dataset_name: str) -> str:
    ...
    #     return f"{prefix}{prompt_text}{suffix}"
        ...
        if len(lines) >= 2:
            description = "\n".join(lines[:-1]).strip()
            func_def = lines[-1].strip()
        else:
            description = prompt_text.strip()
            func_def = ""
        prefix = (
            "Task: Code Generation (MBPP)\n"
            "Instruction: Write a Python function to solve the following problem. "
            "Your answer must be a valid Python code block, wrapped with triple backticks and 'python'."
        )
        # 题目描述和函数定义分开，函数定义单独一行
        prompt = f"{prefix}\nProblem:\n{description}\n\nFunction signature:\n{func_def}\n\nPlease output only the code in the following format:\n```python\n# your code here\n```"
        return prompt
    
    elif dataset_name == 'humaneval':
        prefix = (
            "Task: Python Code Generation (HumanEval)\n"
            "Instruction: Write a Python function to solve the following problem. "
            "Your answer must be a valid Python code block, wrapped with triple backticks and 'python'."
        )
        # 题目描述直接用 prompt_text
        ...
        prefix = (
            "Task: Grade School Math Problem Solving (GSM8K)\n"
            "Instruction: Solve the following math word problem step by step. "
            "In the last line, output your final answer in the format: #### [answer], for example: ### 160\n"
        )
        prompt = f"{prefix}\nProblem:\n{prompt_text}\n\nPlease show your reasoning and finish with the answer line."
        return prompt
    
    elif dataset_name == 'math':
        prefix = (
            "Task: Math Word Problem Solving (MATH)\n"
            "Instruction: Solve the following math problem step by step, showing your reasoning process. "
            "In the last line, output your final answer wrapped in LaTeX box format, for example: $\\boxed{42}$ or $\\boxed{\\$42409}$.\n"
        )
        prompt = f"{prefix}\nProblem:\n{prompt_text}\n\nPlease show your reasoning and finish with the answer line."
        return prompt
...
def process_retrieved_kickstart_text(retrieved_kickstart_text: str, dataset_name: str) -> str:
    ...
    if dataset_name in ['arc','medqa','mmlu', 'hellaswag', 'piqa', 'commonsenseqa']:
        # return "Option. "+retrieved_kickstart_text[3:]
        return retrieved_kickstart_text[3:]
    elif dataset_name in ['gpqa']:
        # print(f"gpqa处理: {retrieved_kickstart_text[11:]}")
        return retrieved_kickstart_text[11:]
    elif dataset_name == "banking77":
        return retrieved_kickstart_text[4:]
    elif dataset_name in ['mbpp','humaneval']:
        # 提取 ### code 和 ### test_list 之间的内容
        code = ""
        if "### code" in retrieved_kickstart_text and "### test_list" in retrieved_kickstart_text:
            code_part = retrieved_kickstart_text.split("### code", 1)[1]
            code = code_part.split("### test_list", 1)[0].strip()
        return code
        ...
        # 去掉最后一行（如果是#### 数字答案）
        lines = retrieved_kickstart_text.strip().splitlines()
        if len(lines) > 1 and lines[-1].strip().startswith("####"):
            text2="\n".join(lines[:-1])
            print(f"gsm8k去掉最后一行：{text2}")
            return text2
        else:
            return retrieved_kickstart_text.strip()
