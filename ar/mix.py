...
def main():
    ...
    parser.add_argument('--max_size', type=int, default=None, help='用于训练/测试划分的最大数据条数上限（优先生效）')
    ...
    for batch_idx in range(num_batches):
        ...
        for strength in strengths_to_test:
            for rank in ranks_to_test:
                ...
                if num_placeholders > 0 and args.dataset in ['arc','banking77','medqa','mmlu','piqa','gpqa','hellaswag','commonsenseqa']:
                    # 选择题等：前面加占位符
                ...
                elif num_placeholders > 0 and args.dataset in ['gsm8k']:
                    mask_id = 126336
                    placeholder_tensor = torch.full(
                        (kickstart_ids.shape[0], num_placeholders),
                        mask_id,
                    ...
                    kickstart_ids = torch.cat([kickstart_ids, placeholder_tensor], dim=1)
